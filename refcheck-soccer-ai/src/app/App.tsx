...
function getFrameTimes(duration: number, incidentSecond?: number): number[] {
  ...
  if (Number.isFinite(incidentSecond)) {
    ...
    return [-1.2, -0.6, -0.2, 0.25, 0.7, 1.2]
      .map((offset) => clamp(center + offset, 0, safeDuration))
      .filter((time, index, arr) => index === 0 || Math.abs(time - arr[index - 1]) > 0.08);
  }

  const count = safeDuration <= 4 ? 5 : 6;
  return Array.from({ length: count }, (_, index) => {
    const pct = count === 1 ? 0.5 : 0.08 + (index * 0.84) / (count - 1);
    return clamp(safeDuration * pct, 0, safeDuration);
  });
}
...
async function describeFrames(
  ...
  const prompt = `You are a soccer video analyst. These are sequential frames from one soccer clip. Read them left-to-right as a timeline.
...
Describe only what is visually observable. Focus on ball location, player body positions, contact, offside line clues, arm position, challenge direction, and whether the key decision moment is visible. Do not make the final officiating verdict yet.`;
